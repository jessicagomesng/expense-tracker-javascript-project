// to do -- use bind function to add event listeners in BudgetDiv
// figure out styling of edit transaction form 
// write methods for sorting  by date
...
document.addEventListener('DOMContentLoaded', function(event) {
    ...
    class BudgetDiv { 
        ...
        render() {
            ...
            // hideTransactions.setAttribute(`hide-budget-transactions-id`, this.id);
            // hideTransactions.setAttribute('class', 'hide-transactions button');
            // displayTransactions.addEventListener('click', function(event) {
            //     event.preventDefault();
            //     fetch(`http://localhost:3000/users/${this.user_id}/budgets/${this.id}/transactions`)
            //     .then(function(response) {
            //         return response.json();
            //     })
            //     .then(function(object) {
            ...
            //     event.preventDefault();
            //     let transactions = document.getElementsByClassName(`budget-${this.id} transactions-container`)[0]
            //     transactions.remove();
    ...
    function listBudgets(budgets) {
        ...
        for (const budget of budgets) {
            ...
            editB.addEventListener('click', function(event) { 
                ...
                editB.parentNode.replaceChild(hideEditForm, editB);
                budgetContent.style.display = 'none';
                let editForm = document.createElement('form');
                editForm.setAttribute('class', 'edit-budget');
                let savingsLabel = document.createElement('label');
            ...
            displayT.addEventListener('click', function(event) {
                ...
                .then(function(object) {
                    ...
                    for (const transaction of object) {
                        ...
                        let editT = document.querySelectorAll(`[transaction-data-id="${transaction.id}"]`)[1]
                        let rowT = document.querySelectorAll(`[transaction-data-id="${transaction.id}"]`)[0]
                        // editT.addEventListener('click', function(event) {
                        //     event.preventDefault();
                        //     let editTransactionForm = document.createElement('form');
                        //     editTransactionForm.setAttribute('transaction-data-id', transaction.id)
                        //     let editDate = document.createElement('input');
                        //     editDate.setAttribute('type', 'date')
                        //     editDate.setAttribute('min', `${budget.start_date}`)
                        //     editDate.setAttribute('max', `${budget.end_date}`)
                        ...
                        //     })
                            ...
                            // editTransactionForm.appendChild(submitEdit);
                            // rowT.innerHTML = '';
                            // rowT.appendChild(editTransactionForm);
                            // rowT.appendChild(cancelEdit);
                        // })
                    }
            ...
            hideT.addEventListener('click', function(event) {
                ...
                let form = document.querySelectorAll('[budget-data-id="10"]')[1]
                form.remove();
                transactionsTitle.remove();
            })
            // renderBudget(budget);
    ...
    function renderTransactions(budget, transactions) {
        ...
        let headers = ['date', 'description', 'amount', 'edit', 'delete'];
        table.setAttribute('class', `transactions-container`);
        table.setAttribute('budget-data-id', budget.id)
        table.innerHTML = '';
        let horizontal = document.createElement('div');
        horizontal.setAttribute('class', 'transactions-header row')
        let colOne = document.createElement('div');
